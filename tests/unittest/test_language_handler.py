...
class TestSortFilesByMainLanguages:
    ...
    @patch('pr_agent.algo.language_handler.get_settings')
    def test_file_extension_not_in_main_extensions(self, mock_get_settings):
        # Mock language_extension_map_org with specific extensions
        mock_settings = MagicMock()
        mock_settings.language_extension_map_org = {
            'python': ['.py'],
            'java': ['.java']
        }
        ...
        files = [
            type('', (object,), {'filename': 'file1.py'})(),
            type('', (object,), {'filename': 'file2.java'})(),
            type('', (object,), {'filename': 'file3.md'})(),  # Not in main_extensions_flat
            type('', (object,), {'filename': 'file4.rb'})(),  # Not in main_extensions_flat
        ]
        
        expected_output = [
            {'language': 'Python', 'files': [files[0]]},
            {'language': 'Java', 'files': [files[1]]},
            {'language': 'Other', 'files': [files[2], files[3]]}
        ]
        
        assert sort_files_by_main_languages(languages, files) == expected_output


    def test_is_valid_file_with_empty_filename(self):
        languages = {'Python': 10}
        files = [
            type('', (object,), {'filename': None})(),
